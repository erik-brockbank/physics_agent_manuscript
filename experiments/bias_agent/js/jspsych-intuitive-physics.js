...
jsPsych.plugins["intuitive-physics"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // NB: remove code that shows background indoor and outdoor image (erikb)
    // TODO consider replacing this with a background color or gradient
    // if (trial.image_cue){
    ...
    var on_bound = false;
    var critical_trial_pause = false;
    // if (trial.image_cue){
    ...
    // var binoculars_img = new Image();
    // binoculars_img.src = 'image/binoculars.png';
    ...
    var draw_canvas = function (myshadow) {
        ...
        // }
        ctx.fillStyle = "rgba(142, 214, 255, 0.5)"; // NB: same as "#8ED6FF"
        ...
        // if critical trial, draw semi-circle that obscures the launch location (erikb)
        // if (trial.criticalTrial) { // keep semi-circle after ball launch
        if (trial.criticalTrial && !key_down && !critical_trial_pause) { // remove semi-circle after ball launch
          // draw semi-circle
          var occluder_quadrant = get_occluder_quadrant(trial.rho);
          draw_occluder(quadrant = occluder_quadrant);

          // reset line and fill styles
          // draw_cloud(.30 * canvas_width, .18 * canvas_height); // 11 o'clock
          // draw_cloud(.56 * canvas_width, .22 * canvas_height); // 1 o'clock
          // draw_cloud(.12 * canvas_width, .33 * canvas_height); // 10 o'clock
          // draw_cloud(.43 * canvas_width, .17 * canvas_height); // 12 o'clock
          // draw_cloud(.63 * canvas_width, .39 * canvas_height); // 2 o'clock
          // draw_cloud(.20 * canvas_width, .52 * canvas_height); // 9 o'clock
          // draw_cloud(.59 * canvas_width, .52 * canvas_height); // 3 o'clock
          // draw_cloud(.37 * canvas_width, .47 * canvas_height); // center (455, 360)

          // draw binoculars
          // ctx.drawImage(binoculars_img, .86 * canvas_width, .58 * canvas_height, .1 * canvas_width, .07 * canvas_height); // (1053, 440, 120, 55)
        ...
        // Only draw the arrow if this is not a critical trial or we're pausing in critical trial (erikb)
        if ((showArrow && !trial.criticalTrial) || (trial.criticalTrial && critical_trial_pause)) {
          if (trial.force == 60.0){
            arrow_color = "#ffad33";
          } else {
            arrow_color = "#ff0000";
        ...
        if (trial.enableDebugDraw){
            ...
            draw_shadow(myshadow);
            draw_world();
        }
        ctx.restore();
    };

    var draw_shadow = function (shadow) {
      if (shadow) {
    ...
    var draw_agent = function(x, y, agent_status) {
      if(agent_status == 'thinking') {
        var fill_color = 'rgba(167, 125, 216, 1.0)';
        var line_color = 'rgba(167, 125, 216, 1.0)';
      }
      else if(agent_status == 'finished') {
        fill_color = 'rgba(3, 171, 180, 1.0)';
        line_color = 'rgba(3, 171, 180, 1.0)';
      }
    ...
    var draw_thought_bubble = function(x, y, rad, agent_status) {
      if(agent_status == 'thinking') {
        var fill_color = 'rgba(167, 125, 216, 0.6)';
        var line_color = 'rgba(167, 125, 216, 0.1)';
      }
      else if(agent_status == 'finished') {
        fill_color = 'rgba(3, 171, 180, 0.6)';
        line_color = 'rgba(3, 171, 180, 0.1)';
      }
      ctx.beginPath();
      ...
      // ctx.strokeStyle = 'black';
      ctx.strokeStyle = line_color;
      ctx.lineWidth = 1.0; // value is in pixels
      ctx.stroke();
    ...
    var draw_thought_space = function(x, y, agent_status) {
      if(agent_status == 'thinking') {
        var fill_color = 'rgba(167, 125, 216, 0.6)';
        var line_color = 'rgba(167, 125, 216, 0.1)';
      }
    ...
    var animate_agent = function() {
      trial.agent_status = 'thinking';
      draw_agent(0.85*canvas_width, 0.55*canvas_height, trial.agent_status);
	    let tmp = thought_tracker.toString(2); // convert to binary representation, then evaluate bitmap length in switch
	    switch (tmp.length) {
        case 1:
          break;
        case 2:
          draw_thought_bubble(0.9*canvas_width, 0.5*canvas_height, 8, trial.agent_status); // small bubble 1
		      break;
        case 3:
          draw_thought_bubble(0.9*canvas_width, 0.5*canvas_height, 8, trial.agent_status); // small bubble 1
          draw_thought_bubble(0.89*canvas_width, 0.45*canvas_height, 10, trial.agent_status); // medium bubble 2
          if (tmp[1] == '1') {
              draw_thought_bubble(0.88*canvas_width, 0.39*canvas_height, 12, trial.agent_status); // large bubble 3
          }
          break;
        case 4:
          draw_thought_bubble(0.9*canvas_width, 0.5*canvas_height, 8, trial.agent_status); // small bubble 1
          draw_thought_bubble(0.89*canvas_width, 0.45*canvas_height, 10, trial.agent_status); // medium bubble 2
          draw_thought_bubble(0.88*canvas_width, 0.39*canvas_height, 12, trial.agent_status); // large bubble 3
          draw_thought_space(0.9*canvas_width, 0.275*canvas_height, trial.agent_status);
          break;
        case 5:
          trial.agent_status = 'finished';
          draw_agent(0.85*canvas_width, 0.55*canvas_height, trial.agent_status);
          draw_thought_bubble(0.9*canvas_width, 0.5*canvas_height, 8, trial.agent_status); // small bubble 1
          draw_thought_bubble(0.89*canvas_width, 0.45*canvas_height, 10, trial.agent_status); // medium bubble 2
          draw_thought_bubble(0.88*canvas_width, 0.39*canvas_height, 12, trial.agent_status); // large bubble 3
    ...
    // NB: this is taken almost without modification from the second response here:
    // https://stackoverflow.com/questions/19541192/how-to-draw-cloud-shape-in-html5-canvas
    // TODO (erikb): reformat this to be in terms of `canvas_width` and `canvas_height`
    var draw_cloud = function(x, y) {
      ctx.beginPath();
    ...
    // Get occluder quadrant based on ball launch position
    var get_occluder_quadrant = function(ball_rho) {
      if(ball_rho < 0.5*Math.PI) {
        return "TR";
      }
      else if(ball_rho < Math.PI) {
        return "TL";
      }
      else if(ball_rho < 1.5*Math.PI) {
        return "BL";
      }
      else if(ball_rho < 2*Math.PI) {
        return "BR";
      }
    };

    // Draw semi-circle occluder around a quarter
    var draw_occluder = function(quadrant) {
      if(quadrant == "TL") {
        var lineEnd = {
          x: canvas_width*trial.center_x_ratio - trial.radius_ratio*canvas_height,
          y: canvas_height*trial.center_y_ratio
        };
        var arcPath = {
          start: Math.PI,
          end: 1.5*Math.PI
        ...
        var lineEnd = {
          x: canvas_width*trial.center_x_ratio,
          y: canvas_height*trial.center_y_ratio - trial.radius_ratio*canvas_height
        };
        var arcPath = {
          start: 1.5*Math.PI,
          end: 0
      ...
      ctx.fillStyle = '#8ED6FF';
      ctx.beginPath();
      ctx.moveTo(canvas_width*trial.center_x_ratio, canvas_height*trial.center_y_ratio);
      ctx.lineTo(lineEnd.x, lineEnd.y);
      ctx.stroke();
      ctx.arc(canvas_width*trial.center_x_ratio, canvas_height*trial.center_y_ratio, trial.radius_ratio*canvas_height, arcPath.start, arcPath.end, false);
      ctx.closePath();
      ctx.fill();
      ctx.stroke();
    };
        ...
        while(obj) {
            ...
            switch(shapeType) {
              case 0: // b2Shape.e_circleShape:
                // only draw the ball if this isn't a mystery trial or it is but subject has launched already (erikb)
                if (!trial.criticalTrial || (trial.criticalTrial && key_down)) {
                  ctx.beginPath();
    ...
    var createWorld = function (user_ball) {
        ...
        if (trial.scene == 'outdoor'){
          // keeping no wind but having outdoor image as background (erikb)
    ...
    var step = function () {
      ...
      if (key_down) {
        // if critical trial, pause for 30 steps while showing ball and launch vector before animating launch
        if (trial.criticalTrial && critical_trial_pause_counter <= 30) { // NB: this number chosen based on trial/error
          critical_trial_pause = true; // we are "in" the pause (used elsewhere to decide what to draw)
          critical_trial_pause_counter += 1;
        }
        else {
          critical_trial_pause = false; // reset pause variable
    ...
    var display_thinking = function(){
      if (thought_tracker<10){
        ...
        ctx.textBaseline = 'middle';
        ctx.textAlign = 'center';
        if (trial.criticalTrial) {
          ctx.fillText("MYSTERY ROUND!", 0.5*canvas_width, 0.5*canvas_height);
        } else {
    ...
    var critical_trial_pause_counter = 0; // counter for steps during critical trial pause (erikb)
    ...
    var end_trial = function(caught) {
      ...
      var trial_data = {
        ...
        agent_cond: trial.agent_cond, // agent condition: 'light', 'heavy' or 'accurate' bias
